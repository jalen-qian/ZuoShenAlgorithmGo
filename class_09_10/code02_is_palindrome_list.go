...
func IsPalindrome2(head *Node) bool {
	...
	// slow就是中点或者下中点，从slow开始反转链表 1->2->3->2->1 中点是3 变成 1->2->3<-2<-1 这里翻转链表，同时让3指向空
	// 如果是偶数个， 1->2->3->4->2->1 下中点是4，反转成 1->2->3->4<-2<-1 同时也让4指向空
	// 极端情况，只有两个，比如 1->2  则slow是2，然后不会发生反转，仍然保持 1->2的结构，下面的c1c2比对的代码，只会比对1次，发现不对等，没毛病
	var pre, next *Node
	cur := slow
	for cur != nil {
		next = cur.Next
