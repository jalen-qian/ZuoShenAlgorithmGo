...
// 3. 如果x节点就是a, b在左右子树中的任意一个，则a、b的最低公共祖先一定是当前的x节点（x节点是b同理）
// 4. 如果a和b有1个既不在左子树，也不在右子树，也不是x，则当前x树一定不存在最低公共祖先。
// 其中：2和4可以统一归纳为：如果左右子树都没找到公共祖先，且当前x树既发现了a也发现了b，则x节点就是最低公共祖先
//
