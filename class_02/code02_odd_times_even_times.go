...
// 将所有的数与m做与运算，如果结果为0，则说明该数在i位置是0，否则是1，这样就能将所有数划分到两个组，且
// a 和 b必然会划分到不同的组去
...
func main() {
	...
	// 6 8 分别出现 3 次和5次，其他数出现偶数次
	arr2 := []int{7, 9, 8, 6, 8, 7, 1, 3, 4, 8, 6, 4, 9, 1, 6, 8, 3, 8}
	num1, num2 := findTowOddTimesNumber(arr2)
	fmt.Printf("arr2中出现奇数次的数为：%d,%d\n", num1, num2)
}
